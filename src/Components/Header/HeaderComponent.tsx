...
export default function HeaderComponent(
  ...
  const menuItems = menuList.map((value: any) => (
    ...
    <Menu
      content={
        ...
        !subMenuValue.newObj?(
              ...
              <MenuItem onClick={() => props.changeMenu({ tabs: [], newObject:value.subMenus[0], item: "item" })}>{subMenuValue.subMenu}</MenuItem>
            </>
            )
      }
    >
    ...
    </Menu>:<Label onClick={() => props.changeMenu({ tabs: [],object:value, item: "menu" })} {...restProps}>
    <span style={menuStyles}>{value.menu}</span>
  </Label>
  ));
  return (
